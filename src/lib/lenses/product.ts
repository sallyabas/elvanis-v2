...
5. Weigh findings by relevance to the client's stated goal (see goalRelevance). Use "directly_supports" for a genuinely healthy/positive finding that is materially and directly relevant to the goal (e.g. a metric comfortably ahead of benchmark, under a goal that metric feeds directly into) — do not force a healthy finding into "directly_blocks" (that's for problems). Use "directly_affects" for a real, material, often-quantifiable cost or drag on the goal that is NOT itself the primary/dominant cause of missing it — this comes up often here: e.g. low activation/adoption wasting real acquisition spend under a cash-flow-efficiency goal is a direct, traceable cost, but reach for "directly_blocks" only when the product issue is genuinely THE central obstruction (e.g. under a churn-retention goal, the specific defect actually driving cancellations). Never invent a value outside the five listed in the schema below — if none feel like a perfect fit, pick the closest of these five rather than making one up. Do not suppress materially important findings just because they're "unrelated" to the goal — surface them, just mark them accordingly.
6. COMPUTED BENCHMARK COMPARISONS ARE FINAL — DO NOT RECOMPUTE. Every metric's tier and comparison-to-benchmark below was already calculated in code, in matching units, and is guaranteed correct. Use each "comparisonText" verbatim (or a light rewording that preserves its exact meaning and direction) in the corresponding finding's "diagnosis" — it's a factual observation, not a causal explanation, so it belongs there, not in "rootCause". Do NOT independently judge whether a value is above/below/within a benchmark, do NOT convert units yourself, and do NOT second-guess the tier — that arithmetic is not your job here. For any number in the evidence that is NOT in the computed comparisons list, you may discuss it qualitatively but must not assert a specific benchmark comparison for it — this means no specific threshold/percentage numbers borrowed from the reference benchmarks section, and also no tier-style language ("concerning," "excellent," "below average," "top quartile," etc.) attached to it, since those are this system's own defined tier names, not generic English. Example of what NOT to do: evidence mentions an NPS or churn figure that was not submitted as a metric — do not write "which is concerning" or "below average"; instead describe it plainly, e.g. "customers have reportedly been less satisfied lately" or "worth watching given the churn signal mentioned," with no tier word and no specific number/threshold attached.
