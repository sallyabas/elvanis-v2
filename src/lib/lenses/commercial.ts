...
5. Weigh findings by relevance to the client's stated goal (see goalRelevance). Use "directly_supports" for a genuinely healthy/positive finding that is materially and directly relevant to the goal (e.g. a competitive position that comfortably outperforms the market, under a goal that competitive strength feeds directly into) — do not force a healthy finding into "directly_blocks" (that's for problems). Use "directly_affects" for a real, material, often-quantifiable cost or drag on the goal that is NOT itself the primary/dominant cause of missing it — this comes up often here: e.g. competitive pricing pressure forcing some discounting under a cash-flow-efficiency goal is a direct, traceable margin cost, but reach for "directly_blocks" only when the competitive dynamic is genuinely THE central obstruction (e.g. a competitor's feature directly causing lost deals under a growth goal). Never invent a value outside the five listed in the schema below — if none feel like a perfect fit, pick the closest of these five rather than making one up. Do not suppress materially important findings just because they're "unrelated" to the goal — surface them, just mark them accordingly.
...
8. ROOT-CAUSE DEPTH (added 2026-08-05, pulled forward from V2 "Better Commercial/Market lens"): a competitive-pressure finding's "rootCause" must explain WHY the competitive dynamic is landing the way it is — not restate that a competitor exists or is aggressive. If the evidence supports it, reason about the client's own competitive differentiation or positioning (e.g. "pricing pressure is landing because the self-report describes no stated differentiation from [competitor] beyond price" or "the lost-deal notes suggest the client is competing on price alone in a market where [competitor] already occupies that position") rather than a shallow "competitor X is undercutting on price." Only reason this deeply when the evidence actually supports it — do not fabricate a differentiation narrative the evidence doesn't contain; a genuinely thin evidence base should stay a thin, honestly-scoped finding (per rule 4), not be padded out with invented strategic analysis.
